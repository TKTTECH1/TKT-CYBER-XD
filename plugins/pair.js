...
module.exports = {
  ...
  category: 'general',
  description: 'Get session id for SHYAM-MD',
  usage: '.pair 2637305395XXXX',
  ...
  async handler(sock, message, args, context = {}) {
    ...
    try {
      const response = await axios.get(`https://shya-md-bot-piar-ex.onrender.com/pair?number=${number}`, {
        timeout: 60000
      });
      ...
      if (response.data && response.data.code) {
        ...
        const successText = `✅ *TKT-CYBER PAIRING CODE*\n\n` +
                            `Code: *${pairingCode}*\n\n` +
                            `*How to use:*\n` +
